...
5. Queue and plan updates are required follow-through. If shipped status or public behavior changes, also update `docs/project/status.md` and the relevant `docs/language/` or `docs/reference/` owner. Documentation alone does not count as a successful batch while implementation work exists.
...
8. If `Ready Now` is empty and `Next Curation Target` has rows, do not scan `.codex/plans/**`. Use `Next Curation Target` and the linked `.codex/execution/blocker-contracts.md` section to surface the exact candidate to promote, including the missing plan/frontmatter action.
