...
- [ ] Add Node.js 22, npm, and `actionlint` to the Nix development shell while retaining the compiler toolchain. Add `website/node_modules/`, `website/build/`, and `website/.docusaurus/` to `.gitignore`.
...
- [ ] The deploy job must declare `needs: build`, use the `github-pages` environment and `actions/deploy-pages@v4`, and expose the returned `page_url` as the environment URL. The dependency is mandatory because the build job owns the Pages artifact upload.
