...
- [x] Implement `scripts/check-website.sh` as a strict Bash entrypoint that runs the Python boundary check, `npm --prefix website run typecheck`, `npm --prefix website run build`, and a second boundary check against generated output. Every package command explicitly targets `website/`.
...
- [x] Add Node.js 22, npm, and `actionlint` to the Nix development shell while retaining the compiler toolchain. Add `website/node_modules/`, `website/build/`, and `website/.docusaurus/` to `.gitignore`.
...
- [x] Create a 1200×630 social-preview composition in SVG with large Jazz branding, the one-line language promise, ample quiet space, and no screenshots or UI frames. Implement `website/scripts/render-social-card.mjs` with `sharp`, requiring exactly 1200×630 output, deterministic encoder options, caller-provided paths, and isolated temporary outputs. Record `npm --prefix website run render:brand` as the explicit regeneration command in `website/static/img/brand/README.md`.
...
- [x] Validate SVG resource references, visible canvas bounds, favicon rasters, and the 1200×630 social card. The brand suite renders the tracked SVG to an independent temporary PNG and requires it to match the committed deployment raster, preventing a changed SVG from deploying with a stale social card.
...
- [x] The build job uses Ubuntu, immutable action revisions for `actions/checkout@v4`, `actions/setup-node@v4`, `actions/configure-pages@v5`, and `actions/upload-pages-artifact@v3`, Node 22 with the npm lockfile cache, package commands with `working-directory: website`, the generated-output boundary check, and artifact path `website/build`.

- [x] The deploy job declares `needs: build`, uses the `github-pages` environment and an immutable `actions/deploy-pages@v4` revision, and exposes the returned `page_url` as the environment URL.
...
- [x] Validate workflow syntax structurally and run the same build locally:
  ...
  ```bash
  nix develop --command actionlint .github/workflows/docs-pages.yml
