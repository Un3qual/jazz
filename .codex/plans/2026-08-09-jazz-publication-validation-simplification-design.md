...
| No remote runtime resources                                  | A post-build scan checks resource-bearing HTML/SVG attributes, CSS functions/imports, and JavaScript fetch/import calls. It permits local/data resources and the production origin while leaving ordinary navigation links alone. This catches accidental fonts, images, stylesheets, scripts, and fetch targets without emulating browsers.        |
| Pinned Actions, least privilege, safe checkout, syntax/order | Pin every `uses:` entry to an immutable commit. A compact Pages contract checks exact actions, job permissions, triggering-repository/SHA checkout without persisted credentials, fail-hard unconditional validation steps, required commands, and deployment order. `actionlint` owns YAML/workflow syntax and runs in Nix-backed CI verification. |
...
- Recursively decoding active HTML/SVG hidden inside `iframe` or `object` data URLs.
- Treating hyperlink `ping` as a publication resource dependency.
- Banning executable inline scripts; Docusaurus emits legitimate inline code, while literal remote `fetch` and `import` calls remain checked.
