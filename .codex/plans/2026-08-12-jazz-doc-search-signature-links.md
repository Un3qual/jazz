...
- Closeout repair: the first website-gate run exposed CSS `url(...)` syntax being applied to generated JavaScript and misclassifying `new URL(...)` parsing bases from the site model and Pagefind. `python3 scripts/test-check-website-boundary.py` failed RED on those parsing bases, then passed after resource contexts became suffix-aware while literal JavaScript `fetch()` and `import()` URLs remained rejected. Commit `df05951b` records the initial repair.
- Fix round 1/5: an inline-script regression proved suffix selection alone still applied CSS matching to an entire HTML document. The focused suite failed RED only for `<script>new URL(...)</script>`, then passed 11/11 after HTML CSS scanning was limited to `<style>` contents plus quoted/unquoted `style=` attributes and JavaScript load scanning was limited to `<script>` contents. The same suite retains rejection coverage for real inline CSS `url(...)` and inline `fetch()`/`import()` resources. The authoritative website gate passed with unchanged production counts.
...
- Final-review identifier fix: literal-span regressions cover `_Maybe`, `Maybe_`, `Maybe'`, `Maybe!`, `ΩMaybe`, and `Ω(a,b)`; before implementation the suite failed RED because `_Maybe` linked the `Maybe` substring and `Ω(a,b)` linked application parentheses as tuple syntax. The splitter now lexes complete identifiers with the public Unicode alphabetic/alphanumeric, `_`, `'`, and `!` grammar, performs exact whole-identifier destination lookup, and reuses the same identifier ends for application-parenthesis classification. `pnpm --dir website run test:signatures` passed 10/10; `scripts/check-docs.sh` passed; and `scripts/check-website.sh` passed with 225 signatures, 683 links, 15 ordinary examples, and 42 indexed pages.
