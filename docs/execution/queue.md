...
Current executor status (`2026-07-21`): the hosted parser and hosted canonical
core are complete through exact repeated stage-0 parity. `Ready Now` is empty.
There is no source-backed next curation target and no named candidate currently.
The next approval gate is a separate backend-neutral lowered-IR design and
plan; LLVM, object/link, and native-runtime implementation remain unpromoted.
...
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `Backend-neutral lowered-IR design approval` | The hosted lexer, parser, and canonical-core milestones are complete. The next ordered gate is a separate reviewed contract for the permanent backend-neutral lowered IR; no lowered-IR, LLVM, object/link, or native-runtime implementation child is currently accepted or promoted. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-21` |
