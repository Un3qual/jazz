...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-TYPED-CORE-EXPRESSION-DIRECT-CALL-001` | Produce typed core and lower scalar direct calls | `P1` | `L` | `impl` | `yes` | `JN-BOOTSTRAP-TYPED-CORE-CONTRACT-FOUNDATION-001`, `JN-BOOTSTRAP-LOWERED-IR-CONTRACT-FOUNDATION-001` | [2026-07-30-jazz-next-typed-core-expression-direct-call.md](../superpowers/plans/2026-07-30-jazz-next-typed-core-expression-direct-call.md) | `Implementation Batch: Typed-Core Expression Production and Direct-Call Lowering` | `docs/execution/blocker-contracts.md`, `docs/execution/done-archive.md`, `docs/execution/queue.md`, `docs/superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md`, `docs/superpowers/specs/2026-07-30-jazz-next-typed-core-expression-direct-call-design.md`, `docs/superpowers/plans/2026-07-30-jazz-next-typed-core-expression-direct-call.md`, `jazz-next/README.md`, `jazz-next/jazz-next.cabal`, `jazz-next/src/JazzNext/Compiler/Analyzer.hs`, `jazz-next/src/JazzNext/Compiler/TypeInference.hs`, `jazz-next/src/JazzNext/Compiler/TypeInference/Diagnostics.hs`, `jazz-next/src/JazzNext/Compiler/TypeInference/Scope.hs`, `jazz-next/src/JazzNext/Compiler/TypedCore/Validate.hs`, `jazz-next/jazz/compiler/TypedCoreValidate.jz`, `jazz-next/test/JazzNext/Compiler/Bootstrap/JazzTypedCoreContractSpec.hs` | Produce and validate one opt-in single-module typed-core scalar/direct-call profile during the existing inference traversal, lower it to validated backend-neutral IR, and prove exact deterministic behavior over the fixed 16-accepted / 20-rejected manifest without changing normal compile/run behavior or permanent mirrored contracts. | `nix --extra-experimental-features 'nix-command flakes' develop -c cabal test --project-dir=jazz-next jazz-typed-core-expression-direct-call-spec jazz-typed-core-contract-spec jazz-lowered-ir-contract-spec --jobs=1 --test-show-details=failures`; `nix --extra-experimental-features 'nix-command flakes' develop -c cabal build --project-dir=jazz-next -fdevelopment all --jobs=1`; `nix --extra-experimental-features 'nix-command flakes' develop -c cabal test --project-dir=jazz-next all --jobs=1 --test-show-details=failures`; `nix --extra-experimental-features 'nix-command flakes' develop -c bash -lc 'cd jazz-next && cabal check'`; `bash scripts/check-execution-queue.sh`; `bash scripts/check-docs.sh`; `git diff --check` | `2026-07-30` |
...
The typed-core expression/direct-call design was approved in discussion and
written form on `2026-07-30`. `Ready Now` contains its sole implementation
child: an opt-in single-pass producer for one resolved-module scalar/direct-call
profile plus deterministic typed-core-to-lowered-IR lowering. Normal
compile/run behavior and the permanent mirrored contracts remain unchanged.
Closure/recursion, control flow, multi-module integration, LLVM, object/link,
...
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `JN-BOOTSTRAP-TYPED-CORE-EXPRESSION-DIRECT-CALL-001 completion` | The hosted lexer, parser, canonical core, backend-neutral lowered-IR contract, typed-core contract foundation, and pre-bootstrap language-quality gate are complete. The approved active child must now produce the first validated scalar/direct-call typed core during inference and lower it to validated IR; closure/recursion, control flow, multi-module integration, LLVM, object/link, and native-runtime implementation remain unpromoted. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-30` |
