...
expression foundation is complete and archived. Types/declarations/modules is
the sole source-backed next curation target; it requires its own reviewed
design and implementation plan before promotion. Control flow/patterns and
operators/full parity remain ordered and unpromoted.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | `JN-BOOTSTRAP-JAZZ-PARSER-TYPES-DECLARATIONS-MODULES-001` | `impl` | [blocker-contracts.md](blocker-contracts.md#jn-bootstrap-interpreter-profile-plan-001) | The contract/kernel and expression-foundation children are complete; the accepted parser ordering names signatures, explicit type applications, declarations, and module syntax as the next coherent grammar/context slice. | `jazz-next/jazz/compiler/ParserToken.jz`, `jazz-next/jazz/compiler/ParserExpression.jz`, `jazz-next/jazz/compiler/ParserProgram.jz`, `jazz-next/jazz/compiler/Parser.jz`, focused bootstrap parser parity support, the shared parser fixture manifest, `jazz-next/jazz-next.cabal`, and directly affected queue/spec/README docs | Written design and implementation-plan review; then `bash scripts/check-execution-queue.sh`; `bash scripts/check-docs.sh`; `git diff --check` | Fix an explicit named fixture family, declaration/module grammar ownership, grammar-context state, exact stage-0 parity boundaries, scale evidence, and focused verification before adding a `Ready Now` row. Do not absorb control flow, patterns, operators, or full-corpus closure. |

## Blocked

| id | title | blocked_on | reason | plan | last_verified |
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `JN-BOOTSTRAP-JAZZ-PARSER-TYPES-DECLARATIONS-MODULES-001` | The hosted lexer, parser contract/kernel, and expression-foundation children are complete. Types/declarations/modules is the sole next curation target but is not executable until a reviewed child design and plan fix its fixture family, grammar-context ownership, and verification. Control-flow/patterns, operators/full parity, canonical core, backend-neutral lowered IR, LLVM lowering, object/link production, and the native runtime remain separate ordered stages. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-16` |
