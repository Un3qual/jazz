...
matching implementation plan fixes its named fixture family, target paths, and
verification. Later parser slices remain ordered and unpromoted.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | `JN-BOOTSTRAP-JAZZ-PARSER-EXPRESSION-FOUNDATION-001` | `impl` | [Bootstrap interpreter blocker contract](blocker-contracts.md#jn-bootstrap-interpreter-profile-plan-001) | The fixed parser schema, structured stage-0 failures, total comparison adapter, and generic parser kernel are complete. The next smallest grammar slice is program/block sequencing, literals, names, applications, lists, tuples, unit, ordinary bindings, and expression statements over one explicit named fixture family. | `jazz-next/jazz/compiler/ParserCore.jz`, `jazz-next/jazz/compiler/ParserTypes.jz`, prospective compiler-local expression/facade modules, focused bootstrap parity tests, and `jazz-next/jazz-next.cabal` | A new implementation plan must name the fixture family, exact files, focused parity command, development build, full suite, queue/docs gates, and `git diff --check`. | Accept a written implementation plan matching the parser design; then promote only this child. Keep types/declarations/modules, control-flow/patterns, and operators/full parity unpromoted. |

## Blocked

| id | title | blocked_on | reason | plan | last_verified |
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `JN-BOOTSTRAP-JAZZ-PARSER-EXPRESSION-FOUNDATION-001 implementation plan` | The hosted lexer and parser contract/kernel foundation are complete. The expression foundation is curated but not promoted: it still needs a reviewed implementation plan with an explicit stable fixture family and exact verification. Types/declarations/modules, control-flow/patterns, operators/full parity, canonical core, backend-neutral lowered IR, LLVM lowering, object/link production, and the native runtime remain separate ordered stages. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-16` |
