...
`Ready Now` is empty after completion of the first hosted bootstrap milestone.
The next curation pass should lock the Jazz-authored parser contract before
promoting implementation, while retaining canonical core and the permanent
backend-neutral lowered-IR-to-LLVM path.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | `JN-BOOTSTRAP-JAZZ-PARSER-DESIGN-001` | `coordination` | [blocker-contracts.md](blocker-contracts.md#jn-bootstrap-interpreter-profile-plan-001) | The Jazz lexer completes the first hosted milestone; the parser is the next compiler component and needs a fixed canonical AST/diagnostic comparison contract plus permanent collection API boundaries before implementation. | `docs/execution/blocker-contracts.md`, `docs/execution/queue.md`, `docs/superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md`, `docs/superpowers/specs/2026-07-12-jazz-next-bootstrap-jazz-parser-design.md` | `bash scripts/check-execution-queue.sh`; `bash scripts/check-docs.sh`; `git diff --check` | Audit the active parser and fixed lexer corpus, accept the canonical parser result schema and recovery/parity rules, name only reusable Jazz collection/text APIs, and preserve the canonical-core -> lowered-IR -> LLVM boundary without bytecode or Haskell callbacks. |

## Blocked

| id | title | blocked_on | reason | plan | last_verified |
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `No accepted compiler child after the hosted lexer` | The first hosted bootstrap milestone is complete: generic named types, ordinary Jazz-authored support modules, backend-neutral text I/O, shared stack-safe evaluation, the canonical comparison corpus, and the Jazz-authored lexer all execute in `jazz-next`. The next source-backed curation target is a Jazz parser design contract; canonical core, backend-neutral lowered IR, LLVM lowering, object/link production, and the native runtime remain permanent separate stages. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-12` |
