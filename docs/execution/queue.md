...
The next source-backed action is a bounded curation/design pass for the
canonical lexer token/diagnostic comparison contract; no implementation child
is promoted until that schema, ownership, target paths, and parity fixtures are
locked.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | `JN-BOOTSTRAP-CANONICAL-COMPARISON-DESIGN-001` | `coordination` | [blocker-contracts.md](blocker-contracts.md#jn-bootstrap-interpreter-profile-plan-001) | The stack-safe evaluator closes implementation slice 6. Slice 7 must lock the stable token, span, lexical-diagnostic, escaping, and normalized-path comparison schema before the Jazz-authored lexer can be implemented or differential parity can be meaningful. | `docs/execution/blocker-contracts.md`, `docs/execution/queue.md`, `docs/superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md`, `docs/superpowers/specs/2026-07-11-jazz-next-bootstrap-canonical-comparison-design.md` | `bash scripts/check-execution-queue.sh`; `bash scripts/check-docs.sh`; `git diff --check` | Audit the current Haskell lexer token/span and diagnostic structures plus accepted/rejected parser fixtures; lock one deterministic Jazz-renderable schema without importing Haskell internals or introducing lowered IR/LLVM concerns. |

## Blocked

| id | title | blocked_on | reason | plan | last_verified |
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `JN-BOOTSTRAP-CANONICAL-COMPARISON-DESIGN-001` | Generic named types, ordinary Jazz-authored `Maybe`/`Result`, scalar `Text` traversal, backend-neutral host text I/O, and shared stack-safe evaluation are complete. The next curation pass must lock the canonical token/diagnostic comparison schema before promoting its implementation or the Jazz-authored lexer; backend-neutral lowered IR, LLVM lowering, and the native runtime remain separate later children. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-11` |
