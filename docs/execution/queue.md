...
No implementation row is currently ready. `JN-BOOTSTRAP-HOST-TEXT-IO-001` is
complete. Curate the stack-safe evaluation contract next; do not promote its
implementation until exact evaluator ownership, tail positions, runtime-host
sequencing, diagnostics, target paths, and verification are accepted.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | `JN-BOOTSTRAP-STACK-SAFE-EVALUATION-CONTRACT-001` | `coordination` | [bootstrap blocker contract](blocker-contracts.md#jn-bootstrap-interpreter-profile-plan-001) | The completed value, generic-module, traversal, and host-I/O boundaries can run Jazz-authored compiler components, but stack-safe evaluator and module recursion across `Runtime`, `ModuleRuntime`, and `Driver` still needs a verified no-host-stack-overflow execution contract. | `docs/superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md`, `docs/execution/blocker-contracts.md`, `docs/execution/queue.md`, `jazz-next/src/JazzNext/Compiler/Runtime.hs`, `jazz-next/src/JazzNext/Compiler/ModuleRuntime.hs`, `jazz-next/src/JazzNext/Compiler/Driver.hs`, `jazz-next/test/JazzNext/Compiler/Semantics/RuntimeSemanticsSpec.hs`, `jazz-next/test/JazzNext/Compiler/Modules/LoaderSpec.hs` | focused evaluator/module evidence review; `bash scripts/check-execution-queue.sh`; `bash scripts/check-docs.sh`; `git diff --check` | Accept exact tail positions, evaluation-loop ownership, host-operation ordering, diagnostic preservation, large-input limits, implementation target paths, and focused tests before promoting an implementation row. |

## Blocked

| id | title | blocked_on | reason | plan | last_verified |
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `JN-BOOTSTRAP-STACK-SAFE-EVALUATION-CONTRACT-001` | Generic named types plus ordinary Jazz-authored `Maybe`, `Result`, scalar `Text` traversal, and backend-neutral host text I/O are complete. Stack-safe tail-position evaluation is the next curation target; lexer/parser modules, canonical comparison output, backend-neutral lowered IR, LLVM lowering, and the native runtime remain separate children. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-11` |
