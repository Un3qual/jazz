...
Current executor status (`2026-07-17`): `Ready Now` is empty. The hosted parser
types/declarations/modules child is complete and archived. Control-flow/patterns
is the sole source-backed next curation target and requires its own reviewed
design and implementation plan before promotion. Operators/full parity remains
ordered behind it and unpromoted.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | `JN-BOOTSTRAP-JAZZ-PARSER-CONTROL-FLOW-PATTERNS-001` | `impl` | [blocker-contracts.md](blocker-contracts.md#jn-bootstrap-interpreter-profile-plan-001) | The contract/kernel, expression, and types/declarations/modules children are complete; the accepted parser ordering names lambdas, conditionals, cases, guards, and patterns as the next coherent grammar slice. | `jazz-next/jazz/compiler/ParserExpression.jz`, `jazz-next/jazz/compiler/ParserDeclaration.jz`, `jazz-next/jazz/compiler/ParserProgram.jz`, focused bootstrap parser parity support, the shared parser fixture manifest, `jazz-next/jazz-next.cabal`, and directly affected queue/spec/README docs | Written design and implementation-plan review; then `bash scripts/check-execution-queue.sh`; `bash scripts/check-docs.sh`; `git diff --check` | Fix one explicit named fixture family, control-flow/pattern ownership, exact accepted and rejected stage-0 boundaries, recursion/context interactions, additive scale evidence, and focused verification before adding a `Ready Now` row. Do not absorb operators or full-corpus closure. |

## Blocked

| id | title | blocked_on | reason | plan | last_verified |
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `JN-BOOTSTRAP-JAZZ-PARSER-CONTROL-FLOW-PATTERNS-001` | The hosted lexer and the first three parser children are complete through signatures, type application, declarations, modules, imports, exports, exact 101-case parity, and deterministic mixed scale evidence. Control-flow/patterns is the sole next curation target but is not executable until a reviewed child design and plan fix its family, ownership, failure boundaries, scale evidence, and verification. Operators/full parity, canonical core, backend-neutral lowered IR, LLVM lowering, object/link production, and the native runtime remain separate ordered stages. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-17` |
