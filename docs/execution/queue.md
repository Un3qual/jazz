...
Current executor status (`2026-07-21`): the backend-neutral lowered-IR contract
foundation is complete and archived. `Ready Now` is empty; there is no
source-backed next curation target and no named candidate currently. Typed-core
elaboration requires a separate reviewed design before core-to-IR lowering;
LLVM, object/link, and native-runtime implementation remain unpromoted.
...
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `No accepted typed-core elaboration design` | The hosted lexer, parser, canonical-core, and backend-neutral lowered-IR contract milestones are complete. Typed-core elaboration must define per-node resolved types, evidence, and representation ownership before core-to-IR lowering can be promoted; LLVM, object/link, and native-runtime implementation remain unpromoted. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-21` |
