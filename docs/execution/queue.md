...
`Ready Now` is empty. `JN-BOOTSTRAP-TEXT-TRAVERSAL-001` completed the ordinary
Jazz-authored scalar traversal layer with private backend-neutral stage-0
adapters. The next bootstrap-profile pass should curate the
`JN-BOOTSTRAP-HOST-TEXT-IO-001` candidate below and lock its module, `IOError`,
purity, runtime, and verification contracts before promotion. Stack-safe
evaluation, the Jazz-authored lexer, backend-neutral lowered IR, LLVM lowering,
and the native runtime remain later unpromoted children.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | `JN-BOOTSTRAP-HOST-TEXT-IO-001` | `impl` | [Bootstrap interpreter profile blocker contract](blocker-contracts.md#jn-bootstrap-interpreter-profile-plan-001) | Literal, generic-type, `Maybe`/`Result`, and in-process scalar traversal slices are complete; a hosted lexer next needs source text to enter and canonical output to leave Jazz through recoverable host effects. | `jazz-next/src/JazzNext/Compiler/BuiltinCatalog.hs`, `jazz-next/src/JazzNext/Compiler/TypeInference.hs`, `jazz-next/src/JazzNext/Compiler/Runtime.hs`, `jazz-next/src/JazzNext/Compiler/Purity.hs`, `jazz-next/test/JazzNext/Compiler/Semantics/{BuiltinCatalogSpec.hs,PrimitiveSemanticsSpec.hs,PuritySemanticsSpec.hs,RuntimeSemanticsSpec.hs}`, `jazz-next/test/JazzNext/Compiler/Modules/LoaderSpec.hs`; the curation pass must additionally lock the exact ordinary Jazz module and `IOError` paths before promotion. | API/error/purity/runtime evidence review; queue/docs gates; `git diff --check` | Reconcile the approved host API with current module/export and stub-purity behavior, then name the Jazz module, `IOError`, bridge, runtime, and focused-test contract without implementing I/O during curation. |

## Blocked

| id | title | blocked_on | reason | plan | last_verified |
| --- | --- | --- | --- | --- | --- |
| `JN-BOOTSTRAP-INTERPRETER-PROFILE-PLAN-001` | Stage the hosted Jazz bootstrap interpreter profile | `No promoted child after Text traversal` | Generic named types plus ordinary Jazz-authored `Maybe`, `Result`, and scalar `Text` traversal are complete through backend-neutral public boundaries. `JN-BOOTSTRAP-HOST-TEXT-IO-001` is the next curation candidate but still needs an accepted module, `IOError`, purity, stage-0 runtime, target-path, and test contract; stack-safe execution, lexer/parser modules, backend-neutral lowered IR, LLVM lowering, and the native runtime remain separate children. | [2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md](../superpowers/specs/2026-07-10-jazz-next-bootstrap-interpreter-profile-design.md) | `2026-07-11` |
