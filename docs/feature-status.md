...
| Type signatures (monomorphic subset) | `Implemented Today`                | `jazz-next/src/JazzNext/Compiler/TypeInference.hs` now parses/enforces adjacent monomorphic signatures over `Int`, `Bool`, nested concrete list types, and single-arrow function types; covered by `jazz-next/test/JazzNext/Compiler/Semantics/BindingSignatureCoherenceSpec.hs` and `jazz-next/test/JazzNext/CLI/CLISpec.hs`. |
