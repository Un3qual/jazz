...
| Pattern matching                   | `Partially Implemented / Parse-Only` | `jazz-next` parses/lowers `case` arms for literal, wildcard, variable, constructor, and bracketed-list patterns; analyzer/type/runtime execution covers literal/wildcard/variable cases, and analyzer/type checking covers declared constructor and bracketed-list patterns (`jazz-next/test/JazzNext/Compiler/Semantics/{AdtPatternTypeSpec.hs,AdtPatternRuntimeSpec.hs}`). Constructor/list pattern runtime matching remains staged per `docs/jazz-language-state.md`. |
