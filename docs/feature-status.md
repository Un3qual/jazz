...
| Pattern matching                   | `Partially Implemented`              | `jazz-next` parses/lowers `case` arms and lambda parameters for literal, wildcard, variable, constructor, bracketed-list, cons-like list, and tuple patterns; analyzer/type/runtime execution covers literal/wildcard/variable, declared constructor, exact-length bracketed-list, cons-like list head/tail, and fixed-arity tuple cases (`jazz-next/test/JazzNext/Compiler/Parser/{AdtPatternParserSpec.hs,LambdaParserSpec.hs}`; `jazz-next/test/JazzNext/Compiler/Semantics/{AdtPatternTypeSpec.hs,AdtPatternRuntimeSpec.hs,LambdaSemanticsSpec.hs}`). |
