...
- Extending primitive semantics coverage beyond the implemented v1 runtime/type subset (`+`, `-`, `*`, `/`, `==`, `!=`, `<`, `<=`, `>`, `>=`, `map`, `filter`, `hd`, `tl`, `print!`, target-named numeric conversions `toInt8`..`toFloat64`, Float64 fractional literal values, same concrete `Float`/`Float64` arithmetic plus comparison/equality, and structural list/tuple equality over equality-supported element types) as the runtime surface expands:
