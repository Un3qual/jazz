...
- Extending primitive semantics coverage beyond the implemented v1 runtime/type subset (`+`, `-`, `*`, `/`, `==`, `!=`, `map`, `filter`, `hd`, `tl`, `print!`, target-named numeric conversions `toInt8`..`toFloat64`, Float64 fractional literal values, and same concrete `Float`/`Float64` arithmetic) as the runtime surface expands:
...
- Extending class/impl abstraction semantics in `jazz-next` beyond the active
  ...
  method bodies, dispatch, dictionaries, default methods, superclass semantics,
  inferred constraints, defaulting, cross-module coherence, and runtime evidence
  remain future work.
...
6. Assume the currently working active implementation (`jazz-next`) is a small interpreter-oriented expression language with:
   ...
   - strict primitive typing/runtime semantics for `+`, `-`, `*`, `/`, `==`, `!=`, plus prelude-provided public helpers `map`, `filter`, `hd`, `tl`, `print!`, target-named numeric conversions `toInt8`..`toFloat64`, default Float64 fractional literal values, and same concrete `Float`/`Float64` arithmetic; numeric-width planning now uses cross-platform `Int64`/`Float64` defaults, context-directed literals, and explicit conversion for mixed concrete widths
