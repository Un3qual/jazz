...
- Extending primitive semantics coverage beyond the implemented v1 runtime/type subset (`+`, `-`, `*`, `/`, `==`, `!=`, `<`, `<=`, `>`, `>=`, `map`, `filter`, `hd`, `tl`, `print!`, target-named numeric conversions `toInt8`..`toFloat64`, Float64 fractional literal values, same concrete `Float`/`Float16`/`Float32`/`Float64` arithmetic, same concrete `Float`/`Float16`/`Float32`/`Float64` comparison/equality, and structural list/tuple equality over equality-supported element types) as the runtime surface expands:
...
6. Assume the currently working active implementation (`jazz-next`) is a small interpreter-oriented expression language with:
   ...
   - strict primitive typing/runtime semantics for `+`, `-`, `*`, `/`, `==`, `!=`, `<`, `<=`, `>`, `>=`, plus prelude-provided public helpers `map`, `filter`, `hd`, `tl`, `print!`, target-named numeric conversions `toInt8`..`toFloat64`, default Float64 fractional literal values, same concrete `Float`/`Float16`/`Float32`/`Float64` arithmetic, same concrete `Float`/`Float16`/`Float32`/`Float64` comparison/equality, and structural list/tuple equality over equality-supported element types, while numeric-width planning now uses cross-platform `Int64`/`Float64` defaults, source-exact fractional literal conversion checks, context-directed literals, and explicit conversion for mixed concrete widths
