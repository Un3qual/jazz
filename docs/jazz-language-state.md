...
- Extending class/impl abstraction semantics in `jazz-next` beyond the active
  ...
  signature-only class method metadata, inert concrete impl method binding
  ...
  reject at parse time, and method dispatch, dictionaries, default methods,
  superclass semantics, inferred constraints, defaulting, cross-module
  coherence, and runtime evidence remain future work. `Self` is not reserved.
