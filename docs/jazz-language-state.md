...
2. Continue module/import closure (`domain 09`) through the remaining active `jazz-next` harness rows: resolution/import binding, then loader/migration. The file-layout parser/resolver harness is complete, and the file layout/package-root, deterministic resolution/cycle, loader pipeline, qualified import, and migration policy specs are published.
