...
2. Continue module/import closure (`domain 09`) through the remaining active `jazz-next` loader/migration harness row. The file-layout parser/resolver and resolution/import-binding harnesses are complete, and the file layout/package-root, deterministic resolution/cycle, loader pipeline, qualified import, and migration policy specs are published.
