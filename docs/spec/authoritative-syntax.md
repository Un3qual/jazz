...
3. **Abstractions**
   ...
   - Active `jazz-next` parser behavior lowers top-level and module-body empty `class`/`impl` declarations, including concrete impl target types.
   - Active `jazz-next` parser behavior rejects non-empty `class`/`impl` bodies with unsupported-syntax diagnostics that name deferred method syntax/semantics.
