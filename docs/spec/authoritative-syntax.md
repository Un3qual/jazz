...
3. **Abstractions**
   ...
   - Active `jazz-next` runtime behavior keeps class/impl declarations inert unless an expression explicitly references a supported `Class::method` key; no dictionary or runtime evidence value is produced.
   ...
   - keep unqualified method dispatch, typed overload selection across multiple
     impls, dictionary passing, default methods, superclass constraints,
     overlap, orphans, bundled-prelude method bodies, module export/import
     method behavior, and runtime evidence out of the single-target dispatch
     slice.
