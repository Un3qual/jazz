...
2. **Modules and imports**
   ...
   - In the active `Jazz` resolver, an explicit symbol list is also a visibility boundary: `import Std::List (map, filter)` exposes only those selected names from that import, and references to other exported names from the same dependency report `E4011`.
...
3. **Abstractions**
   ...
   - Active `Jazz` parser behavior lowers top-level and module-body `class` declarations with explicit class parameter metadata plus signature-only method metadata and concrete `impl` declarations with inert method binding metadata.
   - Active `Jazz` parser behavior rejects class method body/default syntax, non-signature class body items, non-binding impl body items, and method-bearing impl bodies whose target is not concrete.
   - Active `Jazz` analyzer/type behavior rejects duplicate class declarations, rejects duplicate class method signatures, rejects duplicate impl method bindings, rejects duplicate concrete impl facts, validates concrete constrained signatures against known class/impl facts using declared class arity, and validates concrete impl method bodies against the substituted class method signature for explicit single-target qualified dispatch.
   - Active `Jazz` runtime behavior keeps class/impl declarations inert unless an expression explicitly references a supported `Class::method` key; qualified method candidates carry compiler-owned runtime evidence, but no user-visible dictionary or evidence value is produced.
   - Active `Jazz` parser behavior rejects non-canonical top-level and module-body `trait` declarations with unsupported-syntax diagnostics that point future abstraction syntax back to `class`/`impl`.
...
- Historical `trait` examples are non-canonical archival evidence only and should be rewritten to `class` in active docs/spec text. Active `Jazz` must not accept `trait`, even temporarily.
...
- [x] the repository root parser tests aligned to the implemented function/module/lambda slice of the canonical contract.
- [x] the repository root implementation aligned to the implemented function/module/lambda slice of the canonical contract.
- [x] the repository root parser lowers canonical `class`/`impl` declaration forms as inert AST nodes.
- [x] the repository root validates class/impl environments for duplicate class names, duplicate concrete impl facts, and concrete constrained signatures.
- [x] the repository root parser rejects non-empty `class`/`impl` declaration bodies instead of discarding deferred method syntax.
- [x] the repository root parser preserves signature-only class method metadata and rejects duplicate methods plus method body/default syntax.
- [x] the repository root parser preserves inert concrete impl method binding metadata and rejects duplicate, non-binding, or non-concrete method-bearing impl bodies.
- [x] the repository root parser/core AST preserves explicit class parameter metadata for future dispatch.
- [x] the repository root parser rejects non-canonical `trait` declaration forms while preserving ordinary identifier uses.
