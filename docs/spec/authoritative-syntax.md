...
Status: active (module/import and lambda slices are implemented in `jazz-next`; `class` declarations parse with signature-only method metadata, concrete `impl` declarations parse with inert method binding metadata and participate in environment validation, class method body/default syntax and non-binding or non-concrete method-bearing `impl` bodies reject explicitly, non-canonical `trait` declarations reject explicitly with no compatibility path, and abstraction dispatch semantics remain pending)
Locked decisions: 2026-03-02
Primary plan: `docs/plans/spec-cleanup/2026-03-02/decisions/01-authoritative-syntax.md`

Implementation note (2026-06-01): `jazz-next` accepts canonical brace-bodied module declarations (`module A::B { ... }`), canonical lambdas (`\(x) -> expr`) including pattern-shaped parameters such as `_`, `(left, right)`, `[head | tail]`, `Just item`, and `whole @ Just item`, list literals/types, tuple literals, and concrete tuple signature types in the active parser/type/runtime/CLI path. The active parser also lowers top-level and module-body `class` declarations with signature-only method metadata plus concrete `impl` declaration forms such as `class Eq(a) { eq :: a -> a -> Bool. }` and `impl Eq(Int) { eq = \(left, right) -> left == right. }`, preserving method signature payloads, concrete impl target types, and inert impl method body metadata for later phases. Explicit class parameters are the canonical future dispatch model; `Self` is not a reserved type name. Duplicate class declarations reject with `E1004`, duplicate class method signatures reject with `E1006`, duplicate concrete impl facts reject with `E1005`, duplicate impl method bindings reject with `E1007`, and concrete constrained signatures require a visible class declaration plus matching concrete impl fact before normalizing to the monomorphic signature body. Class method body/default syntax, non-binding impl body items, and method-bearing non-concrete impl bodies reject explicitly; method dispatch/runtime abstraction semantics remain future work. Non-canonical `trait ... { ... }` declaration-shaped forms reject with diagnostics that point future abstraction syntax back to `class`/`impl`. `trait` has no active compatibility or deprecation-warning path; ordinary uses of those identifiers as binding names, signature names, or qualified aliases remain valid.
...
3. **Abstractions**
   ...
   - Active `jazz-next` parser behavior lowers top-level and module-body `class` declarations with signature-only method metadata and concrete `impl` declarations with inert method binding metadata.
   ...
   - Active `jazz-next` analyzer/type behavior rejects duplicate class declarations, rejects duplicate class method signatures, rejects duplicate impl method bindings, rejects duplicate concrete impl facts, and validates concrete constrained signatures against known class/impl facts.
...
- [ ] `jazz-next/` parser/core AST preserves explicit class parameter metadata for future dispatch.
