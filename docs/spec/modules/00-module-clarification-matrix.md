...
| Standard library loading | Bundled prelude helpers are loaded by default in module-graph driver helpers and mirrored by `jazz-next/jazz/stdlib/Prelude.jz`. | Keep public prelude helpers separated from `__kernel_*` bridge names. | Tie module specs to the stdlib boundary without expanding the stdlib in this module batch. | Future stdlib closure row if needed. |
