...
- Fractional literal suffix syntax is not supported, integer/fractional operator mixing remains a type error, same concrete `Float32` arithmetic requires operands to resolve to `Float32` through explicit annotation or explicit conversion, and same concrete `Float16`/`Float32` comparison/equality requires operands to resolve to the same concrete width through explicit annotation or explicit conversion.
