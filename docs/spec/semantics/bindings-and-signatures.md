...
- Decimal fractional literals can satisfy explicit `Float`, `Float16`, `Float32`, and `Float64` binding signatures when the literal is the direct binding value; same concrete `Float`/`Float16`/`Float32`/`Float64` arithmetic, comparison, and equality are supported; structural list/tuple equality is supported only when every nested element type is equality-supported, and structural ADT equality is supported only when every declared constructor payload type is equality-supported.
