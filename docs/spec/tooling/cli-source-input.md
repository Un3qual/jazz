...
1. With no positional source file, standalone CLI compile and `--run` read the
   program from stdin.
2. With one positional `.jz` source file, standalone CLI compile and `--run`
   read that file and do not read stdin.
3. Successful standalone compile remains diagnostics-only: stdout and stderr
   are quiet unless warnings or errors are emitted.
4. `--run` evaluates the selected source and writes the rendered result to
...
2. A positional source file cannot be combined with `--entry-module`; module
   graph execution remains selected through `--entry-module` and
   `--module-root`.
3. A missing or unreadable source file is an argument/configuration error and
