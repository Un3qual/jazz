...
| Combining     | `listAppend`, `listConcat`, `listRepeat`, `listIntersperse`, `listIntercalate` | Append is `O(n)` in its left input; repeat and intersperse are `O(k)`; current left-folded concat/intercalate can be quadratic across many fragments |
