...
type instance MetaOf   Parsed = SourceSpan


-- AST -------------------------------------------------------------------------

-- | Parsed names, either qualified or unqualified.
data PName = PUnqual SourceSpan Text
           | PQual   SourceSpan [Text] Text
...
data SourceSpan = SourceSpan SourcePos SourcePos deriving (Show, Eq, Ord)

class HasSpan t where
  range :: t -> SourceSpan

-- instance HasSpan (Module Parsed) where
--   range = modMeta

-- instance HasSpan (ModType Parsed) where
--   range (MTVar     l _)     = l
--   range (MTSig     l _)     = l
--   range (MTFunctor l _ _ _) = l

-- instance HasSpan (Sig Parsed) where
--   range Sig { .. } = sigMeta

-- instance HasSpan (Bind Parsed) where
--   range Bind { .. } = bMeta

-- instance HasSpan (Data Parsed) where
--   range Data { .. } = dMeta

-- instance HasSpan (Syn Parsed) where
--   range Syn { .. } = synMeta

-- instance HasSpan (Constr Parsed) where
--   range Constr { .. } = cMeta

-- instance HasSpan (ModSpec Parsed) where
--   range (MSSig  l _)   = l
--   range (MSKind l _)   = l
--   range (MSData l _)   = l
--   range (MSMod  l _ _) = l

-- instance HasSpan (ModExpr Parsed) where
--   range (MEName       l _)     = l
--   range (MEApp        l _ _)   = l
...
-- instance HasSpan (Match Parsed) where
--   range (MPat   l _ _) = l
--   range (MSplit l _ _) = l
--   range (MFail  l)     = l
--   range (MExpr  l _)   = l

-- instance HasSpan (Schema Parsed) where
--   range (Schema l _ _) = l

-- instance HasSpan (Type Parsed) where
--   range (TCon l _)   = l
--   range (TVar l _)   = l
--   range (TApp l _ _) = l
--   range (TFun l _ _) = l

instance HasSpan (Expr Parsed) where
  range (EVar l _)   = l
  range (ECon l _)   = l
  range (EApp l _ _) = l
  range (EAbs l _ _) = l
...
instance HasSpan (Pat Parsed) where
  range (PVar  l _)   = l
  range (PWild l)     = l
  range (PCon  l _ _) = l

-- instance HasSpan (ModStruct Parsed) where
--   range (ModStruct l _) = l

instance HasSpan (Literal Parsed) where
  range (LInt l _ _) = l

instance HasSpan PName where
  range (PUnqual l _) = l
  range (PQual l _ _) = l
