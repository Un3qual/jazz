...
def normalized_action_versions(contents: str) -> str:
    """Expose approved, immutable action identities to semantic checks."""
...
def check_pr_changes_job(contents: str, violations: list[str]) -> None:
    ...
    if not job:
        ...
        return
    if not re.search(
        r"(?m)^\s*(?:-\s+)?uses:\s+dorny/paths-filter@approved\s*$", job
    ):
        violations.append("changes job must use dorny/paths-filter")
...
def check_pr_docs_job(contents: str, violations: list[str]) -> None:
    ...
    required = (
        (
            r"(?m)^\s*(?:-\s+)?uses:\s*cachix/install-nix-action@approved\s*$",
            "docs-and-site job must install the pinned Nix documentation toolchain",
        ),
        (
            r"(?m)^\s*(?:-\s+)?uses:\s*pnpm/action-setup@approved\s*$",
            "docs-and-site job must use pnpm/action-setup",
        ),
        (
        ...
        (
            r"(?m)^\s*(?:-\s+)?uses:\s*actions/setup-node@approved\s*$",
            "docs-and-site job must use actions/setup-node",
        ),
        (
...
def check_pr_compiler_job(contents: str, violations: list[str]) -> None:
    ...
    requirements = (
        ...
        (
            r"(?m)^\s*(?:-\s+)?uses:\s*cachix/install-nix-action@approved\s*$",
            "compiler-fast job must use cachix/install-nix-action",
        ),
        (
            r"(?m)^\s*(?:-\s+)?uses:\s*actions/cache@approved\s*$",
            "compiler-fast job must use actions/cache",
        ),
        (
            r"(?m)^\s*~/.cabal/store\s*$",
...
def check_main_workflow(root: Path, violations: list[str]) -> None:
    ...
    requirements = (
        ...
        (
            r"(?m)^\s*(?:-\s+)?uses:\s*cachix/install-nix-action@approved\s*$",
            "main ordinary job must use cachix/install-nix-action",
        ),
        (
            r"(?m)^\s*(?:-\s+)?uses:\s*actions/cache@approved\s*$",
            "main ordinary job must use actions/cache",
        ),
        (
            r"(?m)^\s*~/.cabal/store\s*$",
    ...
    if not re.search(
        r"(?m)^\s*uses:\s*actions/upload-artifact@approved\s*$", upload_step
    ):
        violations.append(
...
def check_extended_workflow(root: Path, violations: list[str]) -> None:
    ...
    cache_requirements = (
        (
            r"(?m)^\s*uses:\s*actions/cache@approved\s*$",
            "extended job must use actions/cache",
        ),
        (
            r"(?m)^\s*key:\s*.*\$\{\{\s*runner\.os\s*\}\}.*$",
    ...
    if not re.search(
        r"(?m)^\s*(?:-\s+)?uses:\s*cachix/install-nix-action@approved\s*$", job
    ):
        violations.append("extended job must use cachix/install-nix-action")
    ...
    upload_requirements = (
        ...
        (
            r"(?m)^\s*uses:\s*actions/upload-artifact@approved\s*$",
...
def check_release_workflow(root: Path, violations: list[str]) -> None:
    ...
    requirements = (
        ...
        (
            r"(?m)^\s*(?:-\s+)?uses:\s*pnpm/action-setup@approved\s*$",
            "release job must use pnpm/action-setup",
        ),
        (
            r"(?m)^\s*version:\s*11\.18\.0\s*$",
    ...
    upload_requirements = (
        (
            r"(?m)^\s*uses:\s*actions/upload-artifact@approved\s*$",
            "release workflow must upload verified artifacts with actions/upload-artifact",
        ),
        (
