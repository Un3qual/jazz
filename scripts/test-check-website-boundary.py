...
class WebsiteBoundaryTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.root / "website/src/generated").mkdir(parents=True)
...
""",
        ...
        (self.root / "examples/functions").mkdir(parents=True)
        (self.root / "scripts").mkdir()
        factorial = "factorial 6.\n"
        (self.root / "examples/functions/factorial.jz").write_text(
            factorial, encoding="utf-8"
        )
        (self.root / "scripts/example-cases.tsv").write_text(
            "name\tsources\texpected\targs\n"
            "factorial\texamples/functions/factorial.jz\t720\t"
            "--run examples/functions/factorial.jz\n",
            encoding="utf-8",
        )
        (self.root / "website/src/generated/factorial.ts").write_text(
            "// Generated by website/scripts/sync-factorial.mjs. Do not edit.\n"
            f"export const factorialSource = {json.dumps(factorial)};\n"
    ...
    def test_public_docs_root_must_not_be_a_symlink(self) -> None:
        docs = self.root / "docs"
        external_docs = self.root / "external-docs"
    ...
    def test_generated_factorial_must_match_the_checked_example_byte_for_byte(self) -> None:
        generated = self.root / "website/src/generated/factorial.ts"
        generated.write_text(
            "// Generated by website/scripts/sync-factorial.mjs. Do not edit.\n"
            'export const factorialSource = "factorial 5.\\n";\n',
            encoding="utf-8",
        )
        self.assert_violation(
            "website/src/generated/factorial.ts: generated factorial source does not "
            "match the compiler-backed factorial case"
        )

    def test_generated_factorial_must_match_checked_expected_output(self) -> None:
        generated = self.root / "website/src/generated/factorial.ts"
        generated.write_text(
            generated.read_text(encoding="utf-8").replace(
                'factorialExpectedOutput = "720"',
                'factorialExpectedOutput = "721"',
            ),
            encoding="utf-8",
        )
        self.assert_violation(
    ...
    def test_authored_remote_violations_name_each_rejected_url(self) -> None:
        page = self.root / "website/src/pages/index.tsx"
        page.write_text(
            "const first = 'https://images.example/mark.svg';\n"
            "const second = '//fonts.example/jazz.css';\n",
            encoding="utf-8",
        )
        result = self.run_checker()
        for url in (
            "https://images.example/mark.svg",
            "//fonts.example/jazz.css",
        ):
            self.assertIn(
                "website/src/pages/index.tsx: remote authored reference is not "
                f"allowlisted: {url}",
                result.stdout,
            )

    def test_javascript_comments_are_not_remote_references(self) -> None:
        (self.root / "website/scripts/load.mjs").write_text(
    ...
    def test_generated_html_rejects_effective_remote_duplicate_attributes(self) -> None:
        ...
        build.mkdir()
        (build / "index.html").write_text(
            '<img src="https://images.example/mark.svg" src="/jazz/img/mark.svg">',
            encoding="utf-8",
        )
        self.assert_violation(
            "website/build/index.html: generated output loads a remote resource"
    ...
    def test_generated_html_rejects_remote_inline_style_attributes(self) -> None:
        build = self.root / "website/build"
        build.mkdir()
        (build / "index.html").write_text(
            '<div style="background:url(https://images.example/paper.png)"></div>',
            encoding="utf-8",
        )
        self.assert_violation(
            "website/build/index.html: generated output loads a remote resource"
    ...
    def test_generated_html_treats_malformed_remote_targets_as_violations(self) -> None:
        build = self.root / "website/build"
        build.mkdir()
        (build / "index.html").write_text(
            '<img src="http://[::1">', encoding="utf-8"
        )
        self.assert_violation(
            "website/build/index.html: generated output loads a remote resource"
        )
