...
tests =
  [ ("audits the complete producer fixture manifest", testFixtureManifest),
    ...
    ("retains managed arm failures after scrutinee finalization fails", testManagedScrutineeFailureAccumulation),
    ("does not reuse outer bindings after managed pattern finalization fails", testManagedUnavailablePatternBinder),
    ("keeps arbitrary managed construction failures at the lowerer boundary", testManagedConstructionLowererBoundaries),
    ("rechecks the managed pattern lowerer profile", testManagedPatternLowererProfile),
    ("preserves managed pattern parameter shapes", testManagedPatternParameterShapes),
